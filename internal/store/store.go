...
const filePathSearchQuery = `SELECT f.path
FROM files f
WHERE replace(lower(f.path), '\', '/') LIKE '%' || ? || '%' ESCAPE '\'
ORDER BY CASE
    WHEN replace(lower(f.path), '\', '/') = ? THEN 0
    WHEN substr(replace(lower(f.path), '\', '/'), length(rtrim(replace(lower(f.path), '\', '/'), replace(replace(lower(f.path), '\', '/'), '/', ''))) + 1) = ? THEN 1
    WHEN substr(replace(lower(f.path), '\', '/'), length(rtrim(replace(lower(f.path), '\', '/'), replace(replace(lower(f.path), '\', '/'), '/', ''))) + 1) LIKE ? || '%' ESCAPE '\' THEN 2
    WHEN '/' || replace(lower(f.path), '\', '/') || '/' LIKE '%/' || ? || '%/%' ESCAPE '\' THEN 3
    ELSE 4
END,
length(replace(f.path, '\', '/')) ASC,
replace(f.path, '\', '/') ASC
LIMIT ?`

func (s *Store) SearchFilePaths(ctx context.Context, hints []string, limit int) ([]string, error) {
	normalizedHints := make([]string, 0, len(hints))
	for _, hint := range hints {
		normalizedHints = append(normalizedHints, strings.ReplaceAll(hint, "\\", "/"))
	}
	normalizedHints = lookupValues(normalizedHints)
	if len(normalizedHints) == 0 {
		return []string{}, nil
	}
	limit = retrievalLimit(limit, 8, 16)
	...
	for _, hint := range normalizedHints {
		if err := ctx.Err(); err != nil {
			return nil, fmt.Errorf("file path search: %w", err)
		}
		hint = strings.ToLower(hint)
		escaped := escapeLike(hint)
		rows, err := s.db.QueryContext(ctx, filePathSearchQuery,
			escaped,
			hint,
			hint,
			escaped,
		...
		rows.Close()
		if rowsErr != nil {
			return nil, fmt.Errorf("file path search: %w", rowsErr)
		}
		if len(result) >= limit {
			break
		}
...
func (s *Store) SearchSymbolsInPaths(
	...
	for _, path := range paths {
		normalizedPaths = append(normalizedPaths, strings.ReplaceAll(path, "\\", "/"))
	}
	normalizedPaths = lookupValues(normalizedPaths)
	if len(normalizedPaths) == 0 {
		return []model.RankedCandidate{}, nil
	}
	if len(normalizedPaths) > 8 {
		normalizedPaths = normalizedPaths[:8]
	}
	symbolHints = lookupValues(symbolHints)
	if len(symbolHints) > 16 {
		symbolHints = symbolHints[:16]
	}
	perPathLimit = retrievalLimit(perPathLimit, 8, 8)
	limit = retrievalLimit(limit, 40, 40)
	scanLimit := len(normalizedPaths) * perPathLimit
	pathPlaceholders := strings.TrimSuffix(strings.Repeat("?,", len(normalizedPaths)), ",")
	pathArgs := make([]any, 0, len(normalizedPaths)+3)
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	result := make([]model.RankedCandidate, 0, limit)
	seen := make(map[string]bool, limit)
	perPath := make(map[string]int, len(normalizedPaths))
	appendItems := func(items []model.RankedCandidate) {
		for _, item := range items {
			if len(result) >= limit {
				return
			}
			if seen[item.Handle] || perPath[item.Path] >= perPathLimit {
				continue
			}
	...
	queryItems := func(queryText string, args []any) error {
		if err := ctx.Err(); err != nil {
			return fmt.Errorf("scoped symbol search: %w", err)
		}
		...
		rows.Close()
		if scanErr != nil {
			return fmt.Errorf("scoped symbol search: %w", scanErr)
		}
		appendItems(items)
		return nil
	}

	const symbolOrder = ` ORDER BY CASE WHEN c.kind LIKE '%-outline' OR c.kind = 'test-suite' THEN 1 ELSE 0 END, symbol.confidence DESC, c.end_line - c.start_line ASC, f.path ASC, c.start_line ASC, c.handle ASC LIMIT ?`
	passes := []struct {
		condition string
		args      func(string) []any
	}{
		{condition: `symbol.qualified_name = ?`, args: func(hint string) []any { return []any{hint} }},
		{condition: `lower(symbol.qualified_name) = lower(?)`, args: func(hint string) []any { return []any{hint} }},
		{condition: `lower(symbol.name) = lower(?)`, args: func(hint string) []any { return []any{hint} }},
		{condition: `(lower(symbol.name) LIKE ? || '%' ESCAPE '\' OR lower(symbol.qualified_name) LIKE ? || '%' ESCAPE '\')`, args: func(hint string) []any {
			escaped := escapeLike(strings.ToLower(hint))
			return []any{escaped, escaped}
		}},
	}
	for _, pass := range passes {
		for _, hint := range symbolHints {
			args := append([]any{}, pathArgs...)
			args = append(args, pass.args(hint)...)
			args = append(args, scanLimit)
			queryText := rankedCandidateProjection + `c.symbol_handle IS NOT NULL AND replace(f.path, '\', '/') IN (` + pathPlaceholders + `) AND ` + pass.condition + symbolOrder
			if err := queryItems(queryText, args); err != nil {
				return nil, err
			}
			if len(result) >= limit {
				return result, nil
	...
	if strings.TrimSpace(ftsQuery) != "" && len(result) < limit {
		ftsProjection := `SELECT c.handle, f.path, f.language, c.kind, c.symbol_name, c.signature, c.start_line, c.end_line, c.start_byte, c.end_byte, c.content, c.content_hash, COALESCE(symbol.confidence, 0) FROM chunk_fts JOIN chunks c ON c.handle = chunk_fts.handle JOIN files f ON f.id = c.file_id JOIN symbols symbol ON symbol.handle = c.symbol_handle WHERE c.symbol_handle IS NOT NULL AND replace(f.path, '\', '/') IN (` + pathPlaceholders + `) AND chunk_fts MATCH ? ORDER BY bm25(chunk_fts), CASE WHEN c.kind LIKE '%-outline' OR c.kind = 'test-suite' THEN 1 ELSE 0 END, symbol.confidence DESC, c.end_line - c.start_line ASC, f.path ASC, c.start_line ASC, c.handle ASC LIMIT ?`
		args := append([]any{}, pathArgs...)
		args = append(args, ftsQuery, scanLimit)
		if err := queryItems(ftsProjection, args); err != nil {
			return nil, err
		}
	}
	return result, nil
